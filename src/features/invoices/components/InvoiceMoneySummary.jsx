...
export default function InvoiceMoneySummary({
  ...
  return (
    ...
    >
      {/* الإجمالي */}
      <div className="flex-1 min-w-[160px] border border-gray-300 rounded-md bg-white px-3 py-2 flex flex-col items-center gap-1">
        <span className="font-semibold">الإجمالي</span>
        <span className="text-blue-600 font-medium">{total}</span>
      </div>

      {/* طريقة الدفع */}
      <div className="flex-1 min-w-[180px] border border-gray-300 rounded-md bg-white px-3 py-2 flex flex-col items-center gap-1">
        <span className="font-semibold">طريقة الدفع</span>
        <div className="w-full flex flex-col gap-2 items-center">
          {isEditing ? (
            <>
              <select
                className="w-full border border-gray-300 rounded-md px-2 py-1 text-center text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={paymentValue || ""}
                onChange={(e) =>
                  setEditingInvoice({
                    ...editingInvoice,
        ...
        <div className="w-full">
          {isEditing ? (
            <NumberInput
              ...
              className="w-full h-9 text-center text-sm border border-gray-300 rounded-md"
              onChange={(e) =>
                setEditingInvoice({
                  ...editingInvoice,
